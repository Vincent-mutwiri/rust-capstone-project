...
//wallet setup
    ...
    let mining_address = miner_wallet.get_new_address(Some("Mining Reward"), None)?.assume_checked();
...
let trader_address = trader_wallet.get_new_address(Some("Received"),None)?.assume_checked();
