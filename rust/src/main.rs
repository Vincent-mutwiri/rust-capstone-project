...
fn main() -> bitcoincore_rpc::Result<()> {
    ...
    println!(
        "Connected to Bitcoin Core. Chain: {}",
    ...
    println!(
        "Mined {} blocks (first block reward is now mature)",
        MATURITY_BLOCKS
    );
    ...
    let tx_details: serde_json::Value =
        miner_wallet.call("gettransaction", &[json!(txid), json!(null), json!(true)])?;
    ...
    let input_txid = vin["txid"].as_str().expect("Input missing txid");

    let input_vout = vin["vout"].as_u64().expect("Input missing vout index");

    // Get the previous transaction to find input address and amount
    let prev_tx: serde_json::Value = miner_wallet.call(
        "gettransaction",
    ...
    for output in vout.as_array().unwrap() {
        ...
        let amount = output["value"].as_f64().expect("Output missing value");
